...
class OsFormHelper {
  ...
  public static function toggler_field($name, $label, $is_active, $controlledToggleId = false, $size = 'normal'){
    ...
    if($label) $html.= '<div class="os-form-group os-form-toggler-group">';
  ...
  public static function textarea_field($name, $label, $value = '', $atts = array(), $wrapper_atts = array()){
    $extra_class = (isset($atts['theme']) && ($atts['theme'] == 'bordered')) ? ' os-form-group-bordered' : ' os-form-group-transparent';
    // generate id if not set
    if(!isset($atts['id']) && !isset($atts['skip_id'])) $atts['id'] = self::name_to_id($name, $atts);
    ...
    $html = '<div '.self::atts_string_from_array($wrapper_atts).'>';
      $html.= '<div '.self::atts_string_from_array(array('class' => 'os-form-group os-form-textfield-group os-form-textarea-group'.$extra_class)).'>';
        ...
        $html.= '<textarea type="text" placeholder="'.esc_attr($placeholder).'" name="'.$name.'" '.self::atts_string_from_array($atts, ['class' => 'os-form-control']).'>'.$value.'</textarea>';
      $html.= '</div>';
    $html.= '</div>';
    return $html;
  }
  ...
  public static function text_field($name, $label, $value = '', $atts = array(), $wrapper_atts = array()){
    ...
    $html = '';
    if(!empty($wrapper_atts)) $html = '<div '.self::atts_string_from_array($wrapper_atts).'>';
      $html.= '<div class ="os-form-group os-form-textfield-group'.$extra_class.'">';
        $placeholder = (isset($atts['placeholder']) && !empty($atts['placeholder'])) ? $atts['placeholder'] : $label;
        if($label) $html.= '<label for="'.$atts['id'].'">'.$label.'</label>';
