...
class OsStepsHelper {
  ...
  public static function remove_already_selected_steps(){
    ...
    if(!empty(self::$restrictions['selected_service'])){
      ...
      if($service){
        // more than 1 duration, don't skip, ask for duration to be selected
        if(count($service->get_all_durations_arr()) > 1){
          $remove_service_step = false;
        }
        // needs to select capacity, don't skip, ask for capacity
        if($service->should_show_capacity_selector()){
          $remove_service_step = false;
        }
      }
  ...
  public static function should_step_be_skipped($step_name){
    $skip = false;
      ...
      if((self::$booking_object->full_amount_to_charge() == 0) && (self::$booking_object->deposit_amount_to_charge() == 0) && !OsSettingsHelper::is_env_demo()){
        $skip = true;
      }else{
  ...
  public static function can_step_show_next_btn($step_name, $booking_object = false, $restrictions = false){
    ...
    if($step_name == 'services' && $restrictions && isset($restrictions['selected_service'])){
      ...
      // if need to show capacity selector and durations selector is not required - show next button
      if($selected_service->should_show_capacity_selector() && count($selected_service->get_all_durations_arr()) <= 1) $step_show_btn_rules['services'] = true;
    }
  ...
  public static function prepare_step_locations(){
    ...
    $connected_ids = OsConnectorHelper::get_connected_object_ids('location_id', ['agent_id' => self::$booking_object->agent_id, 'location_id' => self::$booking_object->location_id]);
  ...
  public static function prepare_step_confirmation(){
    ...
    if(self::$booking_object->is_new_record()){
      if(!self::$booking_object->save_from_booking_form()){
        ...
        $error = new WP_Error(LATEPOINT_STATUS_ERROR, $response_html, $error_data);
        return $error;
      }else{
