<div class="latepoint-top-bar-w">
	...
	<a href="#" title="<?php _e('Settings', 'latepoint'); ?>" class="latepoint-top-iconed-link latepoint-mobile-top-menu-trigger"><i class="latepoint-icon latepoint-icon-menu"></i></a>
	<a href="#" title="<?php _e('Search', 'latepoint'); ?>" class="latepoint-top-iconed-link latepoint-mobile-top-search-trigger"><i class="latepoint-icon latepoint-icon-search"></i></a>
	<?php do_action('latepoint_top_bar_before_actions'); ?>
	<a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('bookings', 'pending_approval')); ?>" title="<?php _e('Pending Bookings', 'latepoint'); ?>" class="latepoint-top-iconed-link latepoint-top-notifications-trigger">
		<i class="latepoint-icon latepoint-icon-inbox"></i>
		<?php  
		$count_pending_bookings = OsBookingHelper::count_pending_bookings(OsAuthHelper::get_logged_in_agent_id());
		if($count_pending_bookings > 0) echo '<span class="notifications-count">'.$count_pending_bookings.'</span>'; ?>
	</a>
	<a href="<?php echo $settings_link; ?>" title="<?php _e('Settings', 'latepoint'); ?>" class="latepoint-top-iconed-link latepoint-top-settings-trigger"><i class="latepoint-icon latepoint-icon-settings"></i></a>
	<a href="#" <?php echo OsBookingHelper::quick_booking_btn_html(); ?> title="<?php _e('New Appointment', 'latepoint'); ?>" class="latepoint-mobile-top-new-appointment-btn-trigger latepoint-top-iconed-link"><i class="latepoint-icon latepoint-icon-plus-circle"></i></a>
