    ...
    public function delete(){
      if(isset($this->params['id']) && !empty($this->params['id'])){
        if(OsRemindersHelper::delete($this->params['id'])){
          ...
          $response_html = __('Reminder Removed', 'latepoint');
        }else{
          $status = LATEPOINT_STATUS_ERROR;
          $response_html = __('Error Removing Reminder', 'latepoint');
        }
      }else{
        $status = LATEPOINT_STATUS_ERROR;
        ...
        foreach($this->params['reminders'] as $reminder){
          ...
          if(is_array($validation_errors)){
            ...
            if(OsRemindersHelper::save($reminder)){
              $status = LATEPOINT_STATUS_SUCCESS;
              $response_html = __('Reminder Saved', 'latepoint');
            }else{
              $status = LATEPOINT_STATUS_ERROR;
              $response_html = __('Error Saving Reminder', 'latepoint');
            }
          }
        }
      }else{
