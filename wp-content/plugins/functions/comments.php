...
if (!function_exists('wpqa_comment_question_before')) :
	function wpqa_comment_question_before($commentdata) {
		...
		if (!is_admin() && ($get_post_type_comment == "question" || $get_post_type_comment == "post" || $get_post_type_comment == "page")) {
			...
			if ($get_post_type_comment == "question") {
				...
				if ($pay_answer == "on") {
					...
					if (is_user_logged_in()) {
						...
						$user_login_group = (is_array($user_is_login->roles)?reset($user_is_login->roles):(is_array($user_is_login->caps)?key($user_is_login->caps):""));
			...
			if ($get_post_type_comment == "question" || $get_post_type_comment == "posts") {
...
if (!function_exists('wpqa_comment_question')) :
	function wpqa_comment_question($comment_id) {
		...
		if ($post_type == "question" || $post_type == "posts") {
...
function wpqa_after_add_comment($get_comment,$comment_id,$post_id) {
	...
	if ($get_comment->comment_approved == 1) {
		...
		$comment_user_id = $get_comment->user_id;
		if ($comment_user_id > 0 && ($post_type == "question" || $post_type == "post" || $post_type == "posts")) {
			if ($post_type == "question") {
...
if (!function_exists('wpqa_comment_attachment')) :
	function wpqa_comment_attachment($out,$type,$comment_id = 0,$get_post = object) {
		...
		if ((is_singular("question") || (isset($post) && isset($post->post_type) && 'question' == $post->post_type)) || (is_singular("posts") || (isset($post) && isset($post->post_type) && 'posts' == $post->post_type))) {
...
/* Anonymously answer */
add_filter("comment_form_field_comment","wpqa_comment_anonymously_answer");
...
/* Private answer */
add_filter("wpqa_comment_fields","wpqa_comment_private_answer",3,4);
add_filter("wpqa_edit_comment_fields","wpqa_comment_private_answer",3,4);
function wpqa_comment_private_answer($fields,$type,$comment_id = 0,$get_post = object) {
	...
	$private_answer = wpqa_options("private_answer");
	if (isset($post) && isset($post->post_type) && 'question' == $post->post_type && $private_answer == "on") {
		if ($comment_id > 0) {
...
/* Terms and privacy */
add_filter("comment_form_field_comment","wpqa_comment_terms");
...
/* Comment captcha */
add_filter("wpqa_comment_fields","wpqa_comment_captcha",4);
...
function wpqa_can_add_answer($return,$user_id,$custom_permission,$roles,$post) {
	...
	if ($post_type != 'question' || ($post_type == 'question' && $yes_new != 1)) {
		if (($post_type != 'question' && ($is_super_admin || $custom_permission != "on" || (is_user_logged_in() && $custom_permission == "on" && isset($roles["add_comment"]) && $roles["add_comment"] == 1) || (!is_user_logged_in() && $add_comment == "on"))) || ($post_type == 'question' && ($is_super_admin || $custom_permission != "on" || (is_user_logged_in() && $custom_permission == "on" && isset($roles["add_answer"]) && $roles["add_answer"] == 1) || (!is_user_logged_in() && $add_answer == "on")))) {
			$yes_special = 1;
			if (have_comments()) {
				$yes_special = 0;
			}else {
			...
			if ($post_type == 'question' && $yes_special == 1) {
				...
				if ($post_type == 'question' && !$is_super_admin && $answer_per_question == "on" && $user_id > 0 && isset($answers_question) && is_array($answers_question) && !empty($answers_question)) {
					$edit_delete = '';
			...
			$no_allow_to_answer = false;
			if (!is_user_logged_in()) {
				echo '<div id="respond"><div class="alert-message warning"><i class="icon-flag"></i><p>'.($post_type == 'question'?esc_html__("You must login to add an answer.","wpqa"):esc_html__("You must login to add a comment.","wpqa")).'</p></div>'.do_shortcode("[wpqa_login]").'<div class="pop-footer pop-footer-comments">'.wpqa_signup_in_popup().'</div></div>';
			}else {
	...
	if ($no_allow_to_answer == false) {
		...
		if (isset($closed_post) && $closed_post == 1) {
			...
			$return = false;
			echo '<div id="respond"><div class="alert-message warning"><i class="icon-flag"></i><p>'.($post_type == 'question'?esc_html__("You must login to add an answer.","wpqa"):esc_html__("You must login to add a comment.","wpqa")).'</p></div>'.do_shortcode("[wpqa_login]").'</div>';
		}else {
			if ($post_type == "question") {
				$return = false;
...
/* Add replies button */
add_action("wpqa_action_show_replies","wpqa_action_show_replies",1,2);
add_action(wpqa_theme_name."_action_show_replies","wpqa_action_show_replies",1,2);
function wpqa_action_show_replies($comment,$answer) {
...
if (!function_exists('wpqa_process_edit_comments')) :
	function wpqa_process_edit_comments() {
		...
		if ($form_type == "edit_comment") {
			...
			$comment_limit = (int)wpqa_options(($get_post_type_comment == "question"?"answer_limit":"comment_limit"));
			$comment_min_limit = (int)wpqa_options(($get_post_type_comment == "question"?"answer_min_limit":"comment_min_limit"));
