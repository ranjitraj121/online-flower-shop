...
if (!function_exists('wpqa_private')) :
	function wpqa_private($post_id,$first_user,$second_user) {
		...
		$set_private = apply_filters("wpqa_set_private_question",false,$post_id,$first_user,$second_user);
		if ($set_private == true || $get_private_question == 1 || $get_private_question == "on" || ($user_id != "" && $user_id > 0 && $user_is_comment != true)) {
			$yes_private = 0;
...
if (!function_exists('wpqa_question_sticky_area')) :
	function wpqa_question_sticky_area($post_id,$user_id,$anonymously_user,$post_author) {
		...
		if ($wpqa_allow_to_sticky == true) {
			echo '<a href="#" ta class="pay-to-sticky wpqa-open-click color" data-class="pay-to-sticky-area">'.esc_html__("Pay for sticky question","wpqa").'</a>
			<div class="clearfix"></div>
			<div class="pay-to-sticky-area wpqa-open-div wpqa_hide">
				<a href="'.wpqa_checkout_link("sticky",$post_id).'" target="_blank" class="button-default">'.esc_html__("Pay for sticky question","wpqa").'</a>
			</div>
			<div class="clearfix"></div>';
		}
		echo '</div>';
...
if (!function_exists('wpqa_question_list_details')) :
	function wpqa_question_list_details($post,$user_id,$anonymously_user,$question_edit,$question_delete,$question_close,$closed_question,$active_reports,$active_logged_reports,$moderators_permissions) {
		...
		echo '<ul class="question-link-list">';
			...
			$edit   = ($is_super_admin || ((($user_id == $post_author && $post_author > 0) || ($anonymously_user == $user_id && $post_author > 0) && $question_edit == "on")) || (isset($yes_for_category) && $yes_for_category == true && isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")?true:false);
			$delete = ($is_super_admin || ((($user_id == $post_author && $post_author > 0) || ($anonymously_user == $user_id && $post_author > 0)) && $question_delete == "on") || (isset($yes_for_category) && $yes_for_category == true && isset($moderators_permissions['delete']) && $moderators_permissions['delete'] == "delete")?true:false);
			$close  = ($is_super_admin || ((($user_id == $post_author && $post_author > 0) || ($anonymously_user == $user_id && $post_author > 0)) && $question_close == "on")?true:false);
	...
	function wpqa_action_meta_save($posted,$post) {
		if ($post->post_type == 'question') {
			...
			if (empty($get_question_user_id) && isset($posted[wpqa_theme_name.'_question_category'])) {
				$get_term_by = get_term_by('id',(isset($posted[wpqa_theme_name.'_question_category'])?stripslashes($posted[wpqa_theme_name.'_question_category']):""),'question-category');
