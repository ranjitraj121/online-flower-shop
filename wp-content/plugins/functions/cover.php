...
function wpqa_cover_image() {
	...
	if ($cover_image == "on" && wpqa_is_user_profile() && !wpqa_is_user_edit_profile() && !wpqa_is_user_password_profile() && !wpqa_is_user_privacy_profile() && !wpqa_is_user_withdrawals_profile() && !wpqa_is_user_financial_profile() && !wpqa_is_user_mails_profile() && !wpqa_is_user_delete_profile()) {
		...
		$add_questions = (int)wpqa_count_posts_meta("question",$user_id);
			...
			<div class='wpqa-cover-inner".($cover_fixed == "fixed"?"":" the-main-container")."'>
				<div class='wpqa-cover-content'>
					...
					<div class='wpqa-cover-right'>";
						...
						if ($owner || (!$owner && (isset($moderators_available) && $moderators_available == true)) || ($block_users == "on" && !$owner && $get_current_user_id > 0)) {
							echo "<div class='question-list-details'>
								...
								<ul class='question-link-list'>";
									...
									if ($block_users == "on" && !$owner && $get_current_user_id > 0) {
										...
										echo "<li><span class='small_loader loader_2'></span><a class='".($if_block?"unblock-user":"block-user")."' data-nonce='".wp_create_nonce("block_nonce")."' href='#' data-id='".$user_id."'><i class='".($if_block?"icon-back":"icon-block")."'></i><span>".($if_block?esc_html__("Unblock user","wpqa"):esc_html__("Block user","wpqa"))."</span></a></li>";
									}
						...
						($ask_question_to_users == "on" && !$owner?"<div class='ask-question'><a href='".esc_url(wpqa_add_question_permalink("user"))."' class='button-default ask-question-user'>".esc_html__("Ask","wpqa")." ".$display_name."</a></div>":"").
