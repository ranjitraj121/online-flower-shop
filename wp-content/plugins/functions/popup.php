	...
	function wpqa_head_content($login = "",$its_not_login = false) {
		...
		<div class="put-wrap-pop">
			<?php if (!is_user_logged_in()) {
				...
				if (is_singular('post') && isset($allow_post_types["posts"]["value"]) && $allow_post_types["posts"]["value"] == "posts") {
					$allow_to_see = true;
				}
				if (is_singular('question') && isset($allow_post_types["questions"]["value"]) && $allow_post_types["questions"]["value"] == "questions") {
					$allow_to_see = true;
				}
				if ($activate_need_to_login == "on" && !is_home() && !is_front_page() && ((is_page() && $login_only != "on") || (is_single() && !isset($allow_to_see))) && isset($_COOKIE[wpqa_options("uniqid_cookie").'wpqa_locked_content'])) {
					$show_pop_login = true;
				}
		...
		if ($confirm_email != "yes" && $login != "login") {
			...
			if ($active_message == "on" && (wpqa_is_user_profile() || $active_message_filter == true)) {
				...
				if ($is_super_admin || $custom_permission != "on" || ($custom_permission == "on" && (is_user_logged_in() && !$is_super_admin && isset($roles["send_message"])) || (!is_user_logged_in() && $send_message == "on"))) {
					if ($is_super_admin || (((!is_user_logged_in() && $send_message_no_register == "on") || is_user_logged_in()) && $block_message != "on" && (isset($received_message) && $received_message == "on") && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array($user_id,$user_block_message))))) {
