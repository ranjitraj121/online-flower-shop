...
function wpqa_group_tabs($group_id,$user_id,$is_super_admin,$post_author,$group_moderators) {
	...
	if ($is_super_admin || (isset($view_users_group[$group_privacy]) && $view_users_group[$group_privacy] == $group_privacy) || ($post_author == $user_id && $user_id > 0) || (is_array($group_moderators) && in_array($user_id,$group_moderators))) {
		$list_child = "li";
...
function wpqa_group_invite_users($group_id,$is_super_admin,$post_author,$user_id,$group_invitation,$group_moderators,$group_users_array) {
	if (is_user_logged_in() && ($is_super_admin || ($post_author == $user_id && $user_id > 0) || (($group_invitation == "all" || $group_invitation == "admin_moderators") && is_array($group_moderators) && in_array($user_id,$group_moderators)) || ($group_invitation == "all" && is_array($group_users_array) && in_array($user_id,$group_users_array)))) {?>
