		...
		if (isset($author_id) && $author_id > 0) {
			if ($cover == "") {
				...
				if ($author_page == "grid" || $author_page == "grid_pop" || $author_page == "small" || $author_page == "simple_follow" || $author_page == "columns" || $author_page == "columns_pop") {
					/* questions */
					$questions_count = ($category !== ""?wpqa_count_posts_by_user($author_id,"question","publish",$category):wpqa_count_posts_meta("question",$author_id));

					/* answers */
					$answers_count = wpqa_count_comments_meta("question",$author_id);
				...
				<div class="post-inner">';
					...
					$out .= '<div class="user-content">
						<div class="user-inner">';
											...
											if ((($show_icon == 'on' && $widget == 'widget' && $type_post == "post") || $show_icon != "on")) {
												$out .= '<li class="user-posts">
													<a href="'.esc_url(wpqa_get_profile_permalink($author_id,"posts")).'">
														...
														'.wpqa_count_number($posts_count).' '._n("Post","Posts",$posts_count,"wpqa").'
													</a>
												</li>';
											}
							...
							if (($author_page == "small_grid" || $author_page == "simple_follow") && !$owner) {
								if ($class == "blocking") {
									$out .= wpqa_blocking($author_id,($author_page == "small_grid"?"style_4":""),$owner);
								}else {
						...
						if ($author_page != "small_grid" && $author_page != "grid" && $author_page != "grid_pop" && $author_page != "small" && $author_page != "simple_follow" && $author_page != "columns" && $author_page != "columns_pop") {
							...
							if ($active_moderators == "on") {
								...
								$moderators_available = ($is_super_admin || (isset($if_user_id->roles[0]) && $if_user_id->roles[0] != "administrator" && $pending_posts == true && isset($moderators_permissions['ban']) && $moderators_permissions['ban'] == "ban")?true:false);
							}
							$block_users = wpqa_options("block_users");
							if ($block_users == "on" || (isset($moderators_available) && $moderators_available == true) || ($privacy_email == true && $user_email != "") || ($privacy_social == true && ($facebook || $twitter || $linkedin || $youtube || $vimeo || $pinterest || $instagram)) || ($cover == "" && (!isset($message_button) || (isset($message_button) && $message_button == "")))) {
								$out .= '<div class="social-ul">
									<ul>';
										if ($author_page != "single-author" && ($cover == "" && (!isset($message_button) || (isset($message_button) && $message_button == ""))) && !$owner) {
										...
										if ($author_page != "single-author" && $cover == "" && ((isset($moderators_available) && $moderators_available == true) || $block_users == "on")) {
											if (!$owner && isset($moderators_available) && $moderators_available == true) {
												...
												$out .= "<li class='ban-unban-user'><span class='small_loader loader_2'></span><a class='".($if_ban?"unban-user":"ban-user")."' data-nonce='".wp_create_nonce("ban_nonce")."' href='#' data-id='".$author_id."'><span>".($if_ban?esc_html__("Unban user","wpqa"):esc_html__("Ban user","wpqa"))."</span></a></li>";
											}
					...
					if ($cover == "" && !$owner && $author_page != "single-author" && $author_page != "small_grid" && $author_page != "grid" && $author_page != "grid_pop" && $author_page != "small" && $author_page != "simple_follow" && $author_page != "columns" && $author_page != "columns_pop") {
							...
							if ($group > 0) {
								if ($group_type == "approve_decline") {
									...
									if ($group_type == "profile" || !is_user_logged_in() || (isset($group_moderators) && is_array($group_moderators) && in_array($author_id,$group_moderators))) {
										...
										if (is_super_admin($user_id) || $group_author == $user_id || (is_array($group_moderators) && in_array($user_id,$group_moderators))) {
								...
								if (!$owner) {
									if ($class == "blocking") {
										$out .= wpqa_blocking($author_id,"style_2",$owner);
									}else {
...
if (!function_exists('wpqa_blocking')) :
	function wpqa_blocking($author_id,$block_style = "",$owner = "") {
		$out = "";
		if (is_user_logged_in() && !$owner) {
			$get_block_users = get_user_meta(get_current_user_id(),"wpqa_block_users",true);
			if (isset($get_block_users)) {
				$out .= '<div class="user_block'.($block_style == "style_2"?"_2":"").($block_style == "style_3"?"_3":"").($block_style == "style_4"?"_4":"").(!empty($get_block_users) && in_array($author_id,$get_block_users)?($block_style == "style_4"?" user_block_done":" user_block_yes"):"").'">
					...
					if (!empty($get_block_users) && in_array($author_id,$get_block_users)) {
						$out .= '<a href="#" class="unblock-user-page'.($block_style == "style_2" || $block_style == "style_3" || $block_style == "style_4"?"":" tooltip-n").($block_style == "style_4"?" button-default":"").'" data-nonce="'.wp_create_nonce("block_nonce").'" data-rel="'.(int)$author_id.'" title="'.esc_attr__("Unblock","wpqa").'">';
						...
						$out .= '<a href="#" class="block-user-page'.($block_style == "style_2" || $block_style == "style_3" || $block_style == "style_4"?"":" tooltip-n").($block_style == "style_4"?" button-default":"").'" data-rel="'.(int)$author_id.'" data-nonce="'.wp_create_nonce("block_nonce").'" title="'.esc_attr__("Block","wpqa").'">';
						if ($block_style == "style_2" || $block_style == "style_3" || $block_style == "style_4") {
	...
	function wpqa_get_badge($author_id,$return = "",$points = "",$category_points = "") {
		...
		if ($badges_style == "by_groups_points") {
			if ($author_id > 0) {
				...
				$group_key = (is_array($user_info->roles)?reset($user_info->roles):(is_array($user_info->caps)?key($user_info->caps):""));
...
if (!function_exists('wpqa_get_user_stats')) :
	function wpqa_get_user_stats($wpqa_user_id,$user_stats,$active_points,$show_point_favorite) {
		...
		/* questions */
		$add_questions = wpqa_count_posts_meta("question",$wpqa_user_id);

		/* answers */
		$add_answer = wpqa_count_comments_meta("question",$wpqa_user_id);
