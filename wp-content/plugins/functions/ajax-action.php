...
/* Fixed the counts */
function wpqa_confirm_fixed_counts() {
...
add_action('wp_ajax_wpqa_confirm_fixed_counts','wpqa_confirm_fixed_counts');
			...
			echo "<div class='result-div'>
				<ul>";
					if ($search_type == "answers" || $search_type == "comments") {
						$user_id = get_current_user_id();
						$comments_all = get_comments(array("status" => "approve",'search' => $search_value,'number' => $number,"meta_query" => array('relation' => 'AND',array("key" => "answer_question_user","compare" => "NOT EXISTS"),array("key" => "answer_question_private","compare" => "NOT EXISTS")),'post_type' => ($search_type == "answers"?"question":"post")));
						...
						$terms = get_terms(($search_type == "question-category" || $search_type == "category"?$cat_type:$tag_type),array_merge($exclude,array(
							'orderby'    => "count",
							'order'      => "DESC",
							'number'     => apply_filters(($search_type == "question-category" || $search_type == "category"?"wpqa_cats_per_page":"wpqa_tags_per_page"),2*$number),
							'hide_empty' => 0,
							'search'     => $search_value
						)));
						...
						if ($search_query->have_posts()) :
							...
							while ( $search_query->have_posts() ) : $search_query->the_post();
								$k_search++;
								if ($search_result_number >= $k_search) {
									echo "<li><a".($suggest_questions == "suggest-questions"?" target='_blank'":"")." href='".get_permalink($post->ID)."'>".str_ireplace($search_value,"<strong>".$search_value."</strong>",strip_tags(get_the_title($post->ID)))."</a></li>";
								}else {
