			...
			if ($block_users == "on") {
				...
				if ($get_current_user_id > 0) {
					...
					if (is_array($get_block_users) && !empty($get_block_users)) {
						$author__not_in = array_merge($get_block_users,$user_id);
					}
				}
			}
			$recent_questions_query = new WP_Query(array('author__not_in' => $author__not_in,'date_query' => array(array('after' => $specific_date)),'post_type' => 'question','ignore_sticky_posts' => 1,'cache_results' => false,'no_found_rows' => true,'posts_per_page' => 10));
