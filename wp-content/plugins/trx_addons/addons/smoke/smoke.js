...
(function() {
	...
	$document.on('action.init_trx_addons', function() {
		...
		init_smoke( $smoke.data('trx-addons-smoke') );
	...
	function init_smoke( args ) {
		...
		$document.on( 'action.resize_trx_addons', function() {
			config.viewport.size = [$window.width(), $window.height()];
		} );

		if ( args['smoke_curls'] ) SETTINGS.DENSITY_DIFFUSION = args['smoke_curls'];
		...
		if ( args['bg_color'] ) trx_addons_smoke_set_bg_color( args['bg_color'] );
		if ( args['tint_color'] ) trx_addons_smoke_set_tint_color( args['tint_color'] );
		...
		function prepareCanvas( canvas_dom ) {
			...
			window.addEventListener( "mousemove", function( event ) {
				...
				states[0].dx = 5 * ( event.clientX - states[0].x );
				states[0].dy = 5 * ( event.clientY - states[0].y );
				states[0].x = event.clientX;
				states[0].y = event.clientY;
			} );
			...
			window.addEventListener( "touchmove", function( event ) {
				for ( var touches = event.targetTouches, n = 0; n < touches.length; n++ ) {
					var r = states[n];
					r.moved = r.down;
					r.dx = 8 * ( touches[n].clientX - r.x );
					r.dy = 8 * ( touches[n].clientY - r.y );
					r.x = touches[n].clientX;
					r.y = touches[n].clientY;
				}
			}, false );
			...
			window.addEventListener( "touchstart", function( event ) {
				for ( var touches = event.targetTouches, r = 0; r < touches.length; r++ ) {
					...
					states[r].down = true;
					states[r].x = touches[r].clientX;
					states[r].y = touches[r].clientY;
