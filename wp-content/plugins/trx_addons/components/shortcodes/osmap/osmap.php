...
if ( ! function_exists( 'trx_addons_sc_osmap_load_scripts2' ) ) {
	add_action( 'wp_enqueue_scripts', 'trx_addons_sc_osmap_load_scripts2', TRX_ADDONS_ENQUEUE_SCRIPTS_PRIORITY - 1 );
	function trx_addons_sc_osmap_load_scripts2() {
...
if ( !function_exists( 'trx_addons_sc_osmap_load_scripts' ) ) {
	...
	function trx_addons_sc_osmap_load_scripts( $force = false ) {
		...
		if ( ! $loaded && $preview_elm && $optimize && ! $debug && ! $theme_full ) {
			do_action( 'trx_addons_action_load_scripts_front', false, 'sc_osmap', 2 );
		}
	}
}
