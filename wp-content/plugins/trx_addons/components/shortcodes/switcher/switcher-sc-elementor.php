...
if ( ! function_exists( 'trx_addons_sc_switcher_add_in_elementor' ) ) {
	add_action( 'elementor/widgets/widgets_registered', 'trx_addons_sc_switcher_add_in_elementor' );
...
//						),
		...
		// Register widget
		\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new TRX_Addons_Elementor_Widget_Switcher() );
	}
}
...
if ( ! function_exists( 'trx_addons_sc_switcher_start_catch_output' ) ) {
	...
	function trx_addons_sc_switcher_start_catch_output( $element ) {
		...
		if ( ! empty( $TRX_ADDONS_STORAGE['capture_body'] ) && ! trx_addons_is_preview( 'elementor' ) ) {
