...
if ( ! function_exists( 'trx_addons_woocommerce_attrib_show_in_product_list' ) ) {
	...
	function trx_addons_woocommerce_attrib_show_in_product_list( $args ) {
		...
		if ( ! empty( $args['attributes'] ) && is_object( $product ) ) {
			...
			$need_variations = $args['swap'] > 0 && $product->get_type() == 'variable';
			$html = '';
			foreach( $attributes as $attribute ) {
				$attribute = explode( '=', $attribute );
				if ( ! empty( $attribute[1] ) && taxonomy_exists( $attribute[0] ) ) {
					$type = trx_addons_woocommerce_attrib_get_type( $attribute[0] );
					$attribute_obj = get_taxonomy( $attribute[0] );
					$terms = wc_get_product_terms( $product->get_id(), $attribute[0], array( 'fields' => 'all' ) );
					$output = '';
					foreach ( $terms as $term ) {
						$term_val = get_term_meta( $term->term_id, 'value', true );
						if ( $type == 'image' ) {
							$term_val = empty( $term_val ) ? trx_addons_get_no_image( '', true ) : trx_addons_add_thumb_size( $term_val, trx_addons_get_thumb_size( 'tiny' ) );
						} else if ( $type == 'color' ) {
							$term_val = empty( $term_val ) ? urldecode( $term->slug ) : $term_val;
						}
						$output .= '<span class="trx_addons_tooltip trx_addons_product_attribute_item"'
										. ' data-attribute="' . esc_attr( $attribute[0] ) . '"'
										. ' data-type="' . esc_attr( $type ) . '"'
										. ' data-value="' . esc_attr( urldecode( $term->slug ) ) . '"'
										. ' data-tooltip-text="' . ( $args['action'] == 'filter' ? esc_attr( sprintf( __( 'Filter by %s', 'trx_addons' ), $term->name ) ) : esc_attr( $term->name ) ) . '"'
									. '>'
										. ( $args['action'] != 'none'
					...
					if ( ! empty( $output ) ) {
						$html .= sprintf( '<div class="trx_addons_product_attribute trx_addons_product_attribute_type_%1$s">'
												. '<span class="trx_addons_product_attribute_label">%2$s</span>%3$s'
											. '</div>',
											esc_attr( $type ),
			...
			if ( ! empty( $html ) ) {
				$variations = $need_variations
								? ' data-product-variations="' . esc_attr( json_encode( $product->get_available_variations() ) ) . '"'
								: '';
				trx_addons_show_layout( $html, sprintf( '<div class="trx_addons_product_attributes"%s>', $variations ), '</div>' );
			}
		}
	}
}
