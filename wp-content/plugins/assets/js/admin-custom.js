(function($) {
	...
	if (jQuery('.fixed-counts').length) {
		jQuery(".fixed-counts").on("click",function () {
	    	...
	    	jQuery.post(option_js.ajax_a,"action=wpqa_confirm_fixed_counts&user_id="+user_id,function (result) {
	...
	/* Fixed the site counts */
	if (jQuery('.fixed-site-counts').length) {
		jQuery(".fixed-site-counts").on("click",function () {
	    	var fixed_counts = jQuery(this);
	    	fixed_counts.text(option_js.fixing);
	    	jQuery.post(option_js.ajax_a,"action=wpqa_confirm_fixed_counts",function (result) {
	    		fixed_counts.hide().closest(".section-info").fadeOut(function() {
	    			jQuery(this).remove();
	    		});
	    	});
