...
class AdminForm {
	...
	public static function renderField( $type = 'text', $label = '', $name = '', $value = '', $args = [], $sr_only = false, $info = '' ) {
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- already escaped in form builder
	...
	protected static function buildSelect( $args = [] ) {
		...
		foreach ( $args['choices'] as $current_value => $text ) {
			$field .= '<option value="' . $current_value . '" ' . self::isSelected( $selected, $current_value ) . '>' . $text . '</option>';
		}
	 ...
	 * @return string
	 */
	protected static function isSelected( $selected, $current = true ) {
		$result = '';
	...
	protected static function buildAttributes( $args = [] ) {
		...
		foreach ( $args as $key => $value ) {
			...
			if ( in_array( $key, [ 'type', 'name', 'value', 'class', 'id' ], true ) ) {
				continue;
			}
	...
	public static function renderSettingField( $type = 'text', $label = '', $key = '', $args = [], $sr_only = false, $group = Settings::SETTINGS_GROUP ) {
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- already escaped in field builder
