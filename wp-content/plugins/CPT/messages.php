...
function wpqa_process_new_messages($data) {
	...
	if ($post_type == "send_message") {
		...
		do_action('wpqa_send_message_errors',$errors,$message_add);
		...
		do_action('wpqa_finished_send_message',$post_id,$message_add);
	}
	if ($post_type == "send_message") {
		/* Successful */
		return $post_id;
	...
	if ( $the_query->have_posts() ) {
		while ( $the_query->have_posts() ) {
			...
			if ($message_new == 1 || $message_new == "on" || $message_not_new == "no" && ($user_id == $message_user_id || (is_array($message_user_array) && !empty($message_user_array) && in_array($user_id,$message_user_array)))) {
				...
				if ($seen_message == "on" && $user_id != $post_author && $user_id > 0) {
			...
			if (!isset($_POST["mobile"])) {
				$featured_image_message = discy_options("featured_image_message");
				if ($featured_image_message == "on") {
					$featured_image = get_post_meta($message_id,"_thumbnail_id",true);
					if ($featured_image != "") {
						$img_url = wp_get_attachment_url($featured_image,"full");
						if ($img_url != "") {
								$featured_image_message_lightbox = discy_options("featured_image_message_lightbox");
								$featured_image_message_width = discy_options("featured_image_message_width");
								$featured_image_message_height = discy_options("featured_image_message_height");
								...
								$last_image = discy_get_aq_resize_img($featured_image_message_width,$featured_image_message_height,"",$featured_image);
