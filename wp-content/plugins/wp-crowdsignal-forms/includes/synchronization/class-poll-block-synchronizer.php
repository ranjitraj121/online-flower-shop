<?php
/**
 * File containing the model \WP_CROWDSIGNAL_FORMS\Synchronization\Poll_Block_Synchronizer.
 *
 * @package crowdsignal-forms/Synchronization
 * @since 1.0.0
 */

namespace WP_CROWDSIGNAL_FORMS\Synchronization;
 ...
 *
 * @package WP_CROWDSIGNAL_FORMS\Synchronization
 */
class Poll_Block_Synchronizer {
	 ...
	 *
	 * @since 1.0.0
	 * @var \WP_CROWDSIGNAL_FORMS\Auth\WP_CROWDSIGNAL_FORMS_Api_Authenticator|null
	 */
	 ...
	 *
	 * @since 1.0.0
	 * @var \WP_CROWDSIGNAL_FORMS\Gateways\Api_Gateway_Interface
	 */
	protected $gateway;
	...
	public function synchronize() {
		...
		$this->authenticator = WP_CROWDSIGNAL_FORMS::instance()->get_api_authenticator();

		// if there aren't any CS blocks AND a user code hasn't been requested yet, then there is nothing to sync.
		if ( ! $this->entity_bridge->has_WP_CROWDSIGNAL_FORMS_blocks()
		...
		$this->gateway = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway();
	...
	private function handle_api_sync_exception( $sync_exception ) {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'WP_CROWDSIGNAL_FORMS_poll_sync_exception', $sync_exception, $this );
	}
}
