 ...
 *
 * @since 0.9.0
 * @package WP_CROWDSIGNAL_FORMS\Rest_Api
 **/

namespace WP_CROWDSIGNAL_FORMS\Rest_Api\Controllers;
...
class Polls_Controller {
	...
	public function create_poll( \WP_REST_Request $request ) {
		...
		$resulting_poll = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->create_poll( $poll );
	...
	public function update_poll( \WP_REST_Request $request ) {
		...
		$resulting_poll = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->update_poll( $poll );
		if ( is_wp_error( $resulting_poll ) ) {
			return $resulting_poll;
		}
	...
	public function archive_poll( \WP_REST_Request $request ) {
		...
		$resulting_poll = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->archive_poll( $poll_id );
		if ( is_wp_error( $resulting_poll ) ) {
			return $resulting_poll;
		}
	...
	public function unarchive_poll( \WP_REST_Request $request ) {
		...
		$resulting_poll = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->unarchive_poll( $poll_id );
		if ( is_wp_error( $resulting_poll ) ) {
			return $resulting_poll;
		}
	...
	public function get_polls() {
		return rest_ensure_response( WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->get_polls() );
	}
	...
	public function get_poll( $request ) {
		...
		if ( ! is_numeric( $poll_id ) ) {
			...
			$poll_saved_in_meta = WP_CROWDSIGNAL_FORMS::instance()
		...
		$poll = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->get_poll( $poll_id );
	...
	public function get_poll_results( $request ) {
		$poll_id = $request->get_param( 'poll_id' );
		return rest_ensure_response( WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->get_poll_results( $poll_id ) );
	}
