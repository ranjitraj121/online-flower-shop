...
use WP_CROWDSIGNAL_FORMS\Models\Feedback_Survey;
use WP_CROWDSIGNAL_FORMS\Frontend\Blocks\WP_CROWDSIGNAL_FORMS_Feedback_Block;
...
class Feedback_Controller {
	...
	public function upsert_feedback( \WP_REST_Request $request ) {
		...
		$result = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->update_feedback( $survey );
	...
	public function upsert_feedback_response( \WP_REST_Request $request ) {
		...
		$verifies = WP_CROWDSIGNAL_FORMS_Feedback_Block::verify_nonce( $data['nonce'] );

		if ( ! $verifies ) {
			return new \WP_Error( 'Forbidden' );
		}

		$result = WP_CROWDSIGNAL_FORMS::instance()->get_api_gateway()->update_feedback_response(
			$survey_id,
			$data
		);
