	...
	function wpqa_edit_profile($atts, $content = null) {
		...
		if (!is_user_logged_in()) {
			...
			if ($a['type'] != "delete") {
				...
				$send_email_new_post = wpqa_options("send_email_new_post");
				$send_email_post_groups = wpqa_options("send_email_post_groups");
										...
										<div class="page-section page-section-'.$value_sections["value"].'">
											<div class="page-wrap-content">
												...
												foreach ($edit_profile_items_4 as $key_items_4 => $value_items_4) {
													...
													}else if ($key_items_4 == "send_emails_post" && isset($value_items_4["value"]) && $value_items_4["value"] == "send_emails_post" && $send_email_new_post == "on" && is_array($send_email_post_groups) && isset($user_info->roles[0]) && in_array($user_info->roles[0],$send_email_post_groups)) {
														$out .= '<p class="received_email_post_post_field normal_label'.($unsubscribe_mails_value == "on"?" wpqa_hide":"").'">
															<label for="received_email_post_'.$rand_e.'">
																<span class="wpqa_checkbox"><input type="checkbox" name="received_email_post" id="received_email_post_'.$rand_e.'" value="on" '.checked(esc_attr(isset($_POST["received_email_post"])?$_POST["received_email_post"]:(!empty($_POST) && empty($_POST["received_email_post"])?"":$received_email_post)),"on",false).'></span>
																<span class="wpqa_checkbox_span">'.esc_html__("Do you like to get mails when new posts are added?","wpqa").'</span>
															</label>
														</p>';
	...
	function wpqa_process_edit_profile_form() {
		if (isset($_POST['wpqa_profile_nonce']) && wp_verify_nonce($_POST['wpqa_profile_nonce'],'wpqa_profile_nonce')) {
			...
			$posted = array(
				...
				'received_email_post'     => (isset($_POST['received_email_post']) && $_POST['received_email_post'] != ""?esc_html($_POST['received_email_post']):""),
	...
	function wpqa_save_extra_profile_fields( $user_id ) {
		...
		if (isset($_POST['from_admin']) && $_POST['from_admin'] == "yes") {
			...
			if (wpqa_is_user_mails_profile()) {
				$post_array = array('question_schedules','received_email','received_email_post','unsubscribe_mails','new_payment_mail','send_message_mail','answer_on_your_question','answer_question_follow','notified_reply');
