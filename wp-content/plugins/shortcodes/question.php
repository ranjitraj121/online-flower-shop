...
function wpqa_add_edit_question($type,$popup = false,$user = false) {
	...
	if ($user == "user") {
		...
		if (isset($display_name) && $display_name != "") {
			$out .= '<div class="ask-user-question">'.
				wpqa_get_user_avatar(array("user_id" => $get_user_id,"size" => 42)).
	...
	'<form class="form-post wpqa_form" action="'.$return_url.'" method="post" enctype="multipart/form-data">'.(isset($_POST["form_type"]) && $_POST["form_type"] == $type."_question"?apply_filters('wpqa_'.($user == "user"?"user":$type).'_question',($user == "user"?"user":$type)):"").'
		...
		<p class="form-submit">';
			...
			<input type="submit" value="'.($type == "add"?esc_html__("Publish Your Question","wpqa"):esc_html__("Submit","wpqa")).'" class="button-default button-hide-click">
...
function wpqa_get_question_status($question_id = 0,$user_id = 0) {
	...
	if ($question_publish == "draft") {
		...
		if ($approved_questions == "on") {
			$questions_count = wpqa_count_posts_meta("question",$user_id);
   			...
   			if ($post_type == "question") {
   				...
   				if ($get_post->post_status == "draft") {
					...
					if ($post_author != $get_question_user && $get_question_user > 0) {
						wpqa_notifications_activities($get_question_user,$post_author,"",$get_post->ID,"","add_question_user","notifications","","question");
					}
					if ($user_id > 0) {
...
function wpqa_question($atts, $content = null) {
	...
	if (($custom_permission != "on" && ((isset($user_login_group) && $user_login_group == "wpqa_under_review") || (isset($user_login_group) && $user_login_group == "activation"))) || ($custom_permission == "on" && (is_user_logged_in() && !is_super_admin($user_id) && empty($roles["ask_question"])) || (!is_user_logged_in() && $ask_question != "on"))) {
		...
		if (!is_user_logged_in()) {
			$out .= do_shortcode("[wpqa_login]").(isset($a["popup"]) && $a["popup"] == "popup"?"":"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>");
		}
	}else if (!is_user_logged_in() && $ask_question_no_register != "on") {
		$out .= '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("You must login to ask question.","wpqa").'</p></div>'.do_shortcode("[wpqa_login]").(isset($a["popup"]) && $a["popup"] == "popup"?"":"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>");
		...
		if (!is_user_logged_in() && $pay_ask == "on") {
			...
			'.do_shortcode("[wpqa_login]").(isset($a["popup"]) && $a["popup"] == "popup"?"":"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>");
		}else {
...
function wpqa_edit_question() {
	if (isset($_POST["form_type"]) && $_POST["form_type"] == "edit_question") :
		...
		if (is_wp_error($return)) :
   			...
   			if ($post_status != "draft" || ($post_status == "draft" && isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
				wpqa_session('<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("Edited successfully.","wpqa").'</p></div>','wpqa_session');
			...
			if (isset($question_edit['category']) && $question_edit['category']) {
				...
				if ($category_single_multi == "ajax_2") {
					...
					if (is_array($question_edit['category'])) {
						...
						$cat_ids = array();
						$get_term_by = get_term_by('id',(is_array($question_edit['category'])?end($question_edit['category']):$question_edit['category']),'question-category');
