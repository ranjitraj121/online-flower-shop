...
if (!function_exists('wpqa_signup_jquery')) :
	function wpqa_signup_jquery() {
		...
		$allow_signup_nonce = apply_filters("wpqa_allow_signup_nonce",true);
		if ($allow_login_nonce == true && (!isset($_POST['wpqa_signup_nonce']) || !wp_verify_nonce($_POST['wpqa_signup_nonce'],'wpqa_signup_nonce'))) {
...
function wpqa_after_registration($user_id) {
	...
	$default_options = array("show_point_favorite","received_email","received_email_post","received_message","unsubscribe_mails","new_payment_mail","send_message_mail","answer_on_your_question","answer_question_follow","notified_reply","question_schedules");
...
function wpqa_register_edit_profile_errors($errors,$posted,$sort,$type,$user_id = 0) {
	...
	if ($allow_nickname !== "on" && isset($_POST['nickname']) && $_POST['nickname'] != "") {
		global $wpdb;
		$check_nickname = $wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM $wpdb->users as users, $wpdb->usermeta as meta WHERE users.ID = meta.user_id AND meta.meta_key = 'nickname' AND meta.meta_value = %s AND users.ID <> %d",$_POST['nickname'],$user_id));
		if ($check_nickname > 0) {
	...
	if ($allow_display_name !== "on" && isset($_POST['display_name']) && $_POST['display_name'] != "") {
		global $wpdb;
		$check_display_name = $wpdb->get_var($wpdb->prepare("SELECT COUNT(ID) FROM $wpdb->users WHERE display_name = %s AND ID <> %d",$_POST['display_name'],$user_id));
