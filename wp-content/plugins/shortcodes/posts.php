...
function wpqa_add_edit_group_posts($type,$group_id) {
	...
	'<form class="form-post wpqa_form" method="post" enctype="multipart/form-data">'.(isset($_POST["form_type"]) && $_POST["form_type"] == $type."_posts"?apply_filters('wpqa_'.$type.'_group_posts',$type):"").'
				...
				if ($type == "edit") {
					...
					if ($_thumbnail_id != "") {
						...
						<div class="wpqa-delete-image">
							...
							<div class="button-default wpqa-remove-image" data-name="posts_image" data-type="post_meta" data-id="'.$get_posts.'" data-image="'.$_thumbnail_id.'" data-nonce="'.wp_create_nonce("wpqa_remove_image").'">'.esc_html__("Delete","wpqa").'</div>
		...
		$out .= '<p class="form-submit">';
			...
			<input type="submit" value="'.($type == "add"?esc_html__("Publish Your Post","wpqa"):esc_html__("Submit","wpqa")).'" class="button-default button-hide-click">
...
function wpqa_process_new_group_posts($data) {
	...
	if ($form_type == "add_posts") {
		...
		$data = array(
			'post_content' => ($editor_group_posts == "on"?wpqa_kses_stip($posts_add['content'],"yes",""):wpqa_kses_stip_wpautop($posts_add['content'])),
		...
		update_post_meta($posts_id,$post_meta_stats,0);
...
function wpqa_process_edit_group_posts($data) {
	...
	if ($form_type == "edit_posts") {
		...
		$data = array(
			...
			'post_content' => ($editor_group_posts == "on"?wpqa_kses_stip($posts_edit['content'],"yes",""):wpqa_kses_stip_wpautop($posts_edit['content'])),
