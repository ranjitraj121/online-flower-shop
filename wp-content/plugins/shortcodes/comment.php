...
function wpqa_edit_comment_attr() {
	...
	if (isset($comment_id) && $comment_id != 0 && $get_post) {
		...
		if ($activate_edit == true || $is_super_admin || ($can_edit_comment == "on" && $get_comment->comment_approved == 1 && $comment_user_id == $user_id && $comment_user_id > 0 && ($can_edit_comment_after == 0 || $time_end <= $can_edit_comment_after))) {
			...
			$out .= '<form class="wpqa_form edit-comment-form" method="post"'.($attachment_answer == "on" || $featured_image_answer == "on" || $featured_image_group_post_comments == "on"?' enctype="multipart/form-data"':'').'>'.apply_filters('wpqa_edit_comment','edit_comment').
				...
				<div class="form-inputs clearfix">
					<p>
						...
						if ($comment_editor == "on") {
							...
							ob_start();
							$rand = rand(1,1000);
							wp_editor((isset($_POST['comment'])?wpqa_kses_stip($_POST['comment'],"yes","yes"):wpqa_kses_stip($get_comment->comment_content,"yes","yes")),"comment-".$rand,$settings);
							$comment_contents = ob_get_clean();
						}else {
							$comment_contents = '<textarea name="comment" aria-required="true">'.(isset($_POST['comment'])?wpqa_kses_stip($_POST['comment'],"yes"):wpqa_kses_stip($get_comment->comment_content,"yes")).'</textarea>';
						}
