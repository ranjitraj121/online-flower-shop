...
<div class="section-all-search">
	...
	if ($search_value != "" && ($search_type == "answers" && isset($search_attrs["answers"]["value"]) && $search_attrs["answers"]["value"] == "answers") || ($search_type == "comments" && isset($search_attrs["comments"]["value"]) && $search_attrs["comments"]["value"] == "comments")) {
		...
		$args = array("status" => "approve",'search' => $search_value,'number' => $post_number,'offset' => $offset,"meta_query" => array(array("key" => "answer_question_private","compare" => "NOT EXISTS")),'post_type' => ($search_type == "answers"?"question":"post"));
		$comments_all = get_comments($args);
		if (!empty($comments_all)) {
			$total = wpqa_count_the_comments($search_type,array(),"",$search_value);
				...
				<ol class="commentlist clearfix<?php echo ($pagination_args["total"] > 1?"":" commentlist-no-pagination")?>">
					<?php foreach ($comments_all as $comment) {$k_ad++;
						...
						if ($yes_private == 1) {
								...
								wpqa_comment($comment,"","",(get_post_type($comment->comment_post_ID) == "question"?"answer":"comment"),"",$k_ad,"",array("comment_with_title" => true,"show_replies" => false));?>
							</li>
						<?php }else {?>
							<li class="comment">
