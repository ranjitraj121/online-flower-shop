		...
		echo "<ol class='commentlist clearfix".($messages_query->max_num_pages > 1?" section-message-paged":"")."'>";
			while ( $messages_query->have_posts() ) { $messages_query->the_post();
				...
				if (isset($_GET['wpqa_delete_nonce']) && wp_verify_nonce($_GET['wpqa_delete_nonce'],'wpqa_delete_nonce') && ($message_delete == 1 || $message_delete == "on" || is_super_admin($user_id)) && isset($_GET) && isset($_GET["delete_message"]) && $_GET["delete_message"] == $message_post->ID) {
					wpqa_delete_messages($message_post->ID,$message_author,$user_id,$message_user_id,$message_user_array);
				}?>
				<li class="comment">
					<div class="comment-body clearfix">
						<div class="comment-text">
							<?php do_action("wpqa_action_avatar_link",array("user_id" => (isset($_GET["show"]) && $_GET["show"] == "send"?$message_user_id:$message_author),"size" => 42,"span" => "span","pop" => "pop"));?>
								...
								<div class="comment-meta">
									<div class="comment-author">
										<?php if (isset($_GET["show"]) && $_GET["show"] == "send") {
											...
											if ($message_author > 0) {
												echo '<a href="'.get_author_posts_url($message_author).'">'.$display_name.'</a>';
											}else {
									...
									<ul class="comment-reply">
										<?php if ($message_author > 0 && (empty($_GET["show"]) || (isset($_GET["show"]) && $_GET["show"] != "send"))) {
											...
											if (((is_user_logged_in() && (empty($user_block_message) || (isset($user_block_message) && is_array($user_block_message) && !in_array($user_id,$user_block_message))) && ($block_message != "on" || is_super_admin($user_id)) && isset($received_message) && $received_message == "on"))) {?>
												<li class="message-reply"><a href="#" data-width="690" data-id="<?php echo esc_attr($message_post->ID)?>" data-user-id="<?php echo esc_attr($message_author)?>"><i class="icon-reply"></i><?php esc_html_e("Reply","wpqa");?></a></li>
											<?php }
										}
