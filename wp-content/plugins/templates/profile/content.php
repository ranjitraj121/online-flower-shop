...
if (($profile_last_one == true || $last_one == "questions" || $last_one == "posts" || ($last_one == "favorites" && ($show_point_favorite == "on" || wpqa_is_user_owner())) || ($last_one == "asked" && $ask_question_to_users == "on") || ($last_one == "asked-questions" && $ask_question_to_users == "on" && wpqa_is_user_owner()) || ($pay_ask == "on" && $last_one == "paid-questions") || ($last_one == "followed" && ($show_point_favorite == "on" || wpqa_is_user_owner())) || $last_one == "polls" || $last_one == "followers-questions" || $last_one == "followers-posts")) {
	...
	if ($last_one == "answers" || $last_one == "comments" || $last_one == "followers-answers" || $last_one == "followers-comments") {
		...
		$post_type_for_comments = ($last_one == "answers" || $last_one == "followers-answers"?"question":"post");
		$comments_all = get_comments(array_merge($comments_merge,array("status" => "approve",'number' => $rows_per_page,'offset' => $offset,'post_type' => $post_type_for_comments,"meta_query" => array(array("key" => "answer_question_user","compare" => "NOT EXISTS")))));
		...
		$total = wpqa_count_the_comments(($last_one == "comments" || $last_one == "followers-comments"?"comments":"answers"),$user_ids);
	}else {
		$comments_all = get_comments(array('user_id' => $wpqa_user_id,"status" => "approve",'number' => $rows_per_page,'offset' => $offset,'post_type' => 'question',"meta_query" => array('relation' => 'AND',array("key" => "best_answer_comment","compare" => "=","value" => "best_answer_comment"),array("key" => "answer_question_user","compare" => "NOT EXISTS"))));
