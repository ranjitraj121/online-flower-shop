	...
	if ($activate_pay_to_users == "on") {
		...
		if (isset($_POST["process"]) && $_POST["process"] == "request_money") {
			...
			if (isset($success)) {
				...
				$_SESSION['wpqa_session'] = '<div class="alert-message success"><i class="icon-check"></i><p>'.esc_html__("You have just requested payment of your funds.","wpqa").'</p></div>';
			}else {
		...
		<div class="withdrew_content page-section">
			...
			if ($points_user > 0 && $points_user >= $pay_minimum_points) {
				...
				echo '<div class="alert-message success"><i class="icon-check"></i><p>'.sprintf(esc_html__("%s equals to %s.","wpqa"),"<span>".sprintf(_n("%s Point","%s Points",$pay_minimum_points,"wpqa"),$pay_minimum_points)."</span>","<span>1 ".$currency_code."</span>").'</p></div>';
			}?>
			<div class="current_balance">
				<span><?php echo sprintf(esc_html__("We pay for %s, Your current your balance is","wpqa"),"<span>".number_format($pay_minimum_money)." ".$currency_code."</span>")?></span>
