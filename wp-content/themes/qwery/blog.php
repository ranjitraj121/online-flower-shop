...
if ( function_exists( 'qwery_elementor_is_preview' ) && qwery_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			qwery_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	qwery_new_main_query(
		array(
			'post_type'      => qwery_get_theme_option( 'post_type' ),
			'category'       => qwery_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'qwery_filter_get_template_part', qwery_blog_archive_get_template() ) );
}
