...
if ( ! function_exists( 'qwery_array_insert_after' ) ) {
	function qwery_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		$haystack = qwery_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );
...
if ( ! function_exists( 'qwery_getimagesize' ) ) {
	function qwery_getimagesize( $url ) {
...
if ( ! function_exists( 'qwery_get_embed_video' ) ) {
	function qwery_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			// $embed_video = qwery_make_video_autoplay( $embed_video );
...
if ( ! function_exists( 'qwery_is_external_url' ) ) {
	function qwery_is_external_url( $url ) {
		return qwery_is_url( $url ) && strpos( $url, qwery_remove_protocol_from_url( get_home_url(), true ) ) === false;
	}
}
