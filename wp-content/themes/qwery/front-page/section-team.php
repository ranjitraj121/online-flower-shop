<div class="front_page_section front_page_section_team<?php
	$qwery_scheme = qwery_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $qwery_scheme ) && ! qwery_is_inherit( $qwery_scheme ) ) {
		echo ' scheme_' . esc_attr( $qwery_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( qwery_get_theme_option( 'front_page_team_paddings' ) );
	if ( qwery_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$qwery_css      = '';
		$qwery_bg_image = qwery_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$qwery_anchor_icon = qwery_get_theme_option( 'front_page_team_anchor_icon' );
	$qwery_anchor_text = qwery_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( qwery_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$qwery_css      = '';
			$qwery_bg_mask  = qwery_get_theme_option( 'front_page_team_bg_mask' );
			$qwery_bg_color_type = qwery_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $qwery_bg_color_type ) {
				$qwery_bg_color = qwery_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$qwery_caption = qwery_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $qwery_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $qwery_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $qwery_caption, 'qwery_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$qwery_description = qwery_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $qwery_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $qwery_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $qwery_description ), 'qwery_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! qwery_exists_trx_addons() ) {
						...
						qwery_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
